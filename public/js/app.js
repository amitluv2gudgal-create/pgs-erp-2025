...
// openSalaryPdf (newSalaryId);
// POST /api/salaries/generate

// // after generate returns { id: salaryId, ... }
// if (response && response.id) {
//   // preview immediately
//   openSalaryPdf (response.id);
// }
...
//       <button onclick="requestDelete('${table}', ${row.id})">Delete</button>
//       <button onclick="openSalaryPdf(SALARY_ID)">Open PDF</button>
//       <button onclick="downloadSalaryPdf(SALARY_ID)">Download PDF</button>
...
// public/js/app.js
// Main frontend glue for PGS-ERP
// - Integrates with /api/clients, /api/employees, /api/salaries
// - Works alongside public/js/employees.js and public/js/salaries.js if those are present
// - Defensive: will only define helpers if they don't already exist (so it doesn't clash)
...
(function () {
  ...
  function renderClientsTable(clients) {
    ...
    const thead = document.createElement('thead'); thead.appendChild(header);
    const tbody = document.createElement('tbody');
    (clients || []).forEach(c => {
      const row = el('tr', {}, [
        el('td', { text: c.id }),
        el('td', { text: c.name }),
        el('td', { text: c.address || '' }),
        el('td', { text: c.state || '' }),
        el('td', { text: c.district || '' }),
        el('td', {}, [
          (function(){
            const container = document.createElement('div');
            const btn = el('button', { text: 'View' });
  ...
  function renderEmployeesTable(employees) {
    ...
    (employees || []).forEach(emp => {
      const row = el('tr', {}, [
        el('td', { text: emp.id }),
        el('td', { text: emp.name }),
        el('td', { text: emp.category || '' }),
        el('td', { text: emp.client_id || '' }),
        el('td', { text: emp.gender || '' }),
        el('td', { text: emp.telephone || '' }),
        el('td', {}, [
          (function () {
            const wrap = document.createElement('div');
            const btnEdit = el('button', { text: 'Edit' });
            btnEdit.addEventListener('click', () => {
              // If employees.js provides openEditEmployeeModal, use it, else fallback to a simple fetch+edit
              if (typeof window.openEditEmployeeModal === 'function') {
                window.openEditEmployeeModal(emp);
              } else {
                openInlineEdit(emp.id);
              }
            });
            const btnDel = el('button', { text: 'Delete' });
            ...
            const btnGen = el('button', { text: 'Generate Salary' });
            btnGen.addEventListener('click', async () => {
              // Generate for current month by default
              try {
                const month = (new Date().getMonth() + 1);
                const year = new Date().getFullYear();
                const payload = { employee_id: emp.id, month, year };
                const res = await apiPost('/api/salaries/generate', payload);
                if (res && res.id) {
                  alert('Salary generated (id: ' + res.id + ')');
                  loadSalaries();
                  // preview immediately
                  if (typeof window.openSalaryPdf === 'function') {
                    window.openSalaryPdf(res.id);
                  } else {
                    window.open(`/api/salaries/${res.id}/pdf`, '_blank');
                  }
                } else {
                  alert('Salary generation returned unexpected response.');
                }
              } catch (err) {
                console.error(err);
                alert('Generate salary failed: ' + err.message);
              }
            });
  ...
  function renderSalariesTable(salaries) {
    ...
    (salaries || []).forEach(s => {
      const row = el('tr', {}, [
        el('td', { text: s.id }),
        el('td', { text: s.employee_id }),
        el('td', { text: s.month }),
        el('td', { text: s.year }),
        el('td', { text: s.attendance_days || s.attendanceDays || '' }),
        el('td', { text: fmtINR(s.gross) }),
        el('td', { text: fmtINR(s.net) }),
        el('td', {}, [
          (function () {
            const wrap = document.createElement('div');
            const btnView = el('button', { text: 'View PDF' });
            btnView.addEventListener('click', () => {
              if (typeof window.openSalaryPdf === 'function') window.openSalaryPdf(s.id);
              else window.open(`/api/salaries/${s.id}/pdf`, '_blank');
            });
            const btnDownload = el('button', { text: 'Download' });
            btnDownload.addEventListener('click', () => {
              if (typeof window.downloadSalaryPdf === 'function') window.downloadSalaryPdf(s.id);
              else window.open(`/api/salaries/${s.id}/download`, '_blank');
            });
  ...
  // -------------------------
  // Fallback inline edit modal (very small, used if employees.js does not provide an edit modal)
  // -------------------------
  async function openInlineEdit(empId) {
    try {
      const res = await fetch(`/api/employees/${empId}`);
      const j = await res.json();
      if (!j || !j.employee) { alert('Employee not found'); return; }
      const emp = j.employee;
      const modal = q('#modal') || document.body.appendChild(el('div', { id: 'modal' }));
      modal.innerHTML = `
        <div class="modal-content">
          <h3>Edit Employee ${emp.name || ''}</h3>
          <form id="empEditForm">
            <label>Name<br><input id="e_name" value="${(emp.name||'').replace(/"/g,'&quot;')}" required></label><br>
            <label>Phone<br><input id="e_phone" value="${emp.telephone || ''}"></label><br>
            <label>Gender<br>
              <select id="e_gender">
            ...
            <button type="button" id="e_cancel">Cancel</button>
            <button type="submit">Save</button>
          </form>
        </div>
      `;
      const sel = modal.querySelector('#e_gender');
      if (sel && emp.gender) sel.value = emp.gender;
      modal.querySelector('#e_cancel').addEventListener('click', () => { modal.innerHTML = ''; });
      modal.querySelector('#empEditForm').addEventListener('submit', async (ev) => {
        ev.preventDefault();
        const fd = new FormData();
        fd.append('name', modal.querySelector('#e_name').value);
        fd.append('telephone', modal.querySelector('#e_phone').value);
        fd.append('gender', modal.querySelector('#e_gender').value);
        try {
          const response = await fetch(`/api/employees/${empId}`, { method: 'PUT', body: fd });
          ...
          if (jr && jr.success) {
            ...
            alert('Update failed: ' + (jr && jr.error ? jr.error : 'unknown'));
          }
        } catch (err) {
          console.error(err);
      ...
      console.error(err);
      alert('Failed to load employee for edit: ' + err.message);
    }
  }
  ...
  async function loadClients() {
    try {
      const j = await apiGet('/api/clients');
      const clients = j.clients || j.data || [];
      renderClientsTable(clients);
      // populate client select in create employee modal if exists
      const sel = q('#clientSelectGlobal');
      if (sel) {
  ...
  async function loadEmployees() {
    try {
      const j = await apiGet('/api/employees');
      const employees = j.employees || j.data || [];
      renderEmployeesTable(employees);
    } catch (err) {
      console.error('loadEmployees err', err);
    }
  }

  async function loadSalaries() {
    try {
      const j = await apiGet('/api/salaries');
      const salaries = j.salaries || j.data || [];
      renderSalariesTable(salaries);
    } catch (err) {
      console.error('loadSalaries err', err);
    }
  }

  // -------------------------
  // Create Employee button - use employees.js modal if present, else fallback
  // -------------------------
  function wireCreateEmployeeButton() {
    const btn = q('#btnCreateEmployee');
    if (!btn) return;
    btn.addEventListener('click', () => {
      if (typeof window.openCreateEmployeeModal === 'function') {
        window.openCreateEmployeeModal();
      } else {
        // Basic fallback: open inline create modal
        const modal = q('#modal') || document.body.appendChild(el('div', { id: 'modal' }));
        modal.innerHTML = `
          <div class="modal-content">
            <h3>Create Employee</h3>
            <form id="simpleCreateEmp">
              <label>Name<br><input id="c_name" required></label><br>
              <label>Phone<br><input id="c_phone"></label><br>
              <label>Gender<br>
                <select id="c_gender"><option value="">--select--</option><option value="male">Male</option><option value="female">Female</option><option value="other">Other</option></select>
              </label><br>
              <label>Client ID<br><input id="c_client"></label><br>
              <button type="button" id="c_cancel">Cancel</button>
              <button type="submit">Create</button>
            </form>
          </div>
        `;
        modal.querySelector('#c_cancel').addEventListener('click', () => { modal.innerHTML = ''; });
        modal.querySelector('#simpleCreateEmp').addEventListener('submit', async (ev) => {
          ev.preventDefault();
          const fd = new FormData();
          fd.append('name', modal.querySelector('#c_name').value);
          fd.append('telephone', modal.querySelector('#c_phone').value);
          fd.append('gender', modal.querySelector('#c_gender').value);
          const clientVal = modal.querySelector('#c_client').value;
          if (clientVal) fd.append('client_id', clientVal);
          try {
            // use fetch with formdata (employees controller accepts multipart/form-data)
            const resp = await fetch('/api/employees', { method: 'POST', body: fd });
            ...
            if (jr && jr.success) {
              ...
              alert('Create failed: ' + (jr && jr.error ? jr.error : 'unknown'));
            }
          } catch (err) {
            console.error(err);
  ...
  // -------------------------
  // Define salary PDF preview/download helpers if not already present
  // -------------------------
  if (typeof window.openSalaryPdf === 'undefined') {
    window.openSalaryPdf = async function (salaryId) {
      try {
        const resp = await fetch(`/api/salaries/${salaryId}/pdf`);
        if (!resp.ok) {
  ...
  if (typeof window.downloadSalaryPdf === 'undefined') {
    window.downloadSalaryPdf = async function (salaryId) {
      try {
        const resp = await fetch(`/api/salaries/${salaryId}/download`);
        if (!resp.ok) {
          const txt = await resp.text();
  ...
  function init() {
    // wire create button
    wireCreateEmployeeButton();

    // wire refresh buttons if present
    const btnRefreshClients = q('#btnRefreshClients');
    if (btnRefreshClients) btnRefreshClients.addEventListener('click', loadClients);
    const btnRefreshEmployees = q('#btnRefreshEmployees');
    if (btnRefreshEmployees) btnRefreshEmployees.addEventListener('click', loadEmployees);
    const btnRefreshSalaries = q('#btnRefreshSalaries');
    if (btnRefreshSalaries) btnRefreshSalaries.addEventListener('click', loadSalaries);

    // initial loads (if containers exist)
    if (q('#clientsTable')) loadClients();
    if (q('#employeesTable')) loadEmployees();
    if (q('#salariesTable')) loadSalaries();
  }
