...
export const DB_PATH = process.env.DB_PATH || path.join(__dirname, 'pgs_erp.sqlite');
...
export async function initDB() {
  ...
  await ensureEmployeeColumns(dbInstance);

  return dbInstance;
}

async function ensureEmployeeColumns(db) {
  const cols = await db.all("PRAGMA table_info(employees)");
  const existing = cols.map(c => c.name);

  const toAdd = [
    { name: 'basic_rate', type: 'REAL' },
    { name: 'special_allowance', type: 'REAL' },
    { name: 'hra', type: 'REAL' },
    { name: 'ca', type: 'REAL' },
    { name: 'wa', type: 'REAL' },
    { name: 'educational_allowance', type: 'REAL' },
    { name: 'add_4_hours', type: 'REAL' },
    { name: 'weekly_off_amount', type: 'REAL' },
    { name: 'salary_total_manual', type: 'REAL' },
    { name: 'epf_on_basic', type: 'INTEGER' },
    { name: 'epf_on_basic_plus_da', type: 'INTEGER' },
    { name: 'gender', type: 'TEXT' } // NEW
  ];

  for (const col of toAdd) {
    if (!existing.includes(col.name)) {
      await db.exec(`ALTER TABLE employees ADD COLUMN ${col.name} ${col.type};`);
      console.log(`[db] added employees.${col.name}`);
    }
  }
}