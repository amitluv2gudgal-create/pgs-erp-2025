  ...
  try {
    ...
    if (users.length) {
      ...
      return req.session.save((err) => {
        if (err) {
          console.error('[auth] session save error (users):', err);
...
function requireAdmin(req, res, next) {
  if (!req.session?.user || roleOf(req.session.user.role) !== 'admin') {
...
router.post('/change-password', requireAuth, async (req, res) => {
  try {
    ...
    const role = roleOf(req.session.user.role);
    ...
    await run('UPDATE users SET password = $1 WHERE id = $2', [newHash, req.session.user.id]);
...
router.post('/admin/users/:id/reset-password', requireAdmin, async (req, res) => {
  try {
    ...
    await run('UPDATE users SET password = $1 WHERE id = $2', [hash, id]);
...
router.post('/admin/supervisors/:id/reset-password', requireAdmin, async (req, res) => {
  try {
    ...
    const rows = await query('SELECT id FROM security_supervisors WHERE id = $1', [id]);
    if (!rows.length) return res.status(404).json({ error: 'Supervisor not found' });

    const hash = await bcrypt.hash(String(new_password), 10);
    await run('UPDATE security_supervisors SET password = $1 WHERE id = $2', [hash, id]);
...
router.post('/admin/reset-password', requireAdmin, async (req, res) => {
  try {
    ...
    let rows;
    if (userId) rows = await query('SELECT id FROM users WHERE id = $1', [userId]);
    else rows = await query('SELECT id FROM users WHERE username = $1', [username]);

    if (!rows.length) return res.status(404).json({ error: 'User not found' });
    const targetId = rows[0].id;

    const hash = await bcrypt.hash(newPassword, 10);
    await run('UPDATE users SET password = $1 WHERE id = $2', [hash, targetId]);
    ...
    `);
    await run(
      'INSERT INTO admin_password_resets (admin_id, target_user_id) VALUES ($1, $2)',
